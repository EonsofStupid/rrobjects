...
| Distributed / cluster | 🚧 2026-07-17 — **stages 1–2 landed**. (1) replication: `replicate` verb ships each change with its dense record; a `Replica` follower rebuilds a leader's estate from the stream alone (convergent, idempotent, cursor-resumable, byte-identical vectors). (2) **synchronous quorum-ack**: `Cluster` coordinator learns follower positions from their poll cursors (the poll IS the ack), a `durability:"quorum"` `tx` write blocks until a quorum holds it, and an under-replicated write refuses to falsely ack. Stage 3 = leader lease + failover, then the kill-leader gate. Self-reliant (no openraft) | `connxism` (`ReplEntry`/`replication_batch`), `rro-engine/src/{replica,cluster}.rs` | **13** |
